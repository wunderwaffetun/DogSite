...
let documentHeight = Number(Math.max( document.body.scrollHeight, document.body.offsetHeight))
let maxBodyHeight = 0
if (stateBodyHeight == 'True'){
    for(let key in document.body.children){
        try{
            if(getComputedStyle(document.body.children[key]).height != 'auto'){
                ...
                maxBodyHeight += Number(getComputedStyle(document.body.children[key]).marginTop.slice(0, -2))
                maxBodyHeight += Number(getComputedStyle(document.body.children[key]).marginBottom.slice(0, -2))
            }
        }catch{}
    }
...
function THE_ANIMATION_PAWS_MODULE(first, second, third, fourth, fifth){
